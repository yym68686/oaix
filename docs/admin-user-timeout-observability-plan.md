...
状态：2026-09-09，原调查结束后用户要求继续修改、push并监控发布，现已实现应用与数据库等待观测。实际范围与运行方式见 [管理员查询观测运行说明](admin-query-observability.md)。下文调查事实对应修改前的基线，不能将本次观测增强称作历史超时根因修复。调查基线为 `71049f3e56adbc911909c8493ee8ef70f5594b83`。用户明确要求：无法完全确定根因时，不修改代码，转向系统性可观测性方案。
...
- [x] 保存受限的本地事故证据并编写本方案。
- [x] 实现应用阶段、请求级 acquire/query、PG 等待采样、独立诊断存储与 OTLP 导出。
- [ ] auto_explain、节点资源采样、首行回调和页面采纳结果事件，仍需独立的实现/权限/开销验收。
