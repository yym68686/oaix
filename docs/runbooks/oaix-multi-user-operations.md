...
3. 对 GPT-5.6 检查 `cache_write_reported_requests`、`gpt56_cache_write_missing_requests`、`gpt56_cache_write_unobserved_requests`、`cache_write_token_sources` 和 `gpt56_zero_write_then_read_sequences`。`missing` 表示新解析器确认 usage 中没有写入字段；`unobserved` 是观测字段上线前的历史记录，不能据此判断上游漏报。
4. 查看对应请求的 `cache_write_input_tokens`、`cache_write_tokens_source` 和 `prompt_cache_trace.usage.raw`。source 非空且值为 `0` 是上游明确报告零写入；新 trace 中 `cache_write_reported=false` 才是确认缺失。旧 trace 没有该 marker 时只属于不可观测，OAIX 不会推断计费。
