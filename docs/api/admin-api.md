...
- 我的账号：`GET /api/me`、`GET /api/me/usage`、`GET /api/me/dashboard`、`GET /api/me/concurrency`、`GET /api/me/pool-summary`。仪表盘支持今天、本周、本月、今年和自定义日期范围（`range=today|week|month|year|custom`；自定义时传 `from`、`to`），消费金额是 OAIX 按请求用量和模型费率计算的估算值。
- 仪表盘默认 `today`；`week` 从当地周一零点开始，`month` 从月初开始，`year` 从年初开始。`timezone` 为 IANA 时区，默认 UTC。自定义日期格式为 `YYYY-MM-DD`，包含起止两天（今天截止当前时间）；单天按小时、90 天以内按天、更长按月分桶。概览指标与趋势使用相同范围，空桶没有缓存率；`GET /api/me/concurrency` 始终返回本进程当前调用用户的实时并发。
...
- 请求日志/分析：`GET /admin/requests`、`GET /admin/requests/{request_id}`、`GET /admin/requests/export`、`GET /admin/analytics/models`、`GET /admin/analytics/cache`、`GET /admin/analytics/errors`、`GET /admin/analytics/latency`、`GET /admin/inflight`。`GET /admin/requests` 默认不做大表精确总数统计，传 `include_total=true` 才返回精确总数。
  - 请求日志与 CSV 导出包含 `input_tokens`、`cache_write_input_tokens`、`cache_write_tokens_source`、`cached_input_tokens`、`output_tokens`。`cache_write_tokens_source` 非空且写入量为 `0` 表示上游明确报告了零写入；source 为空表示字段缺失，系统不会推断写入量。
  - `GET /admin/analytics/cache` 额外返回写入 token 总量、字段报告/正值请求数、字段来源分布、GPT-5.6 字段缺失数、历史不可观测数，以及 `gpt56_zero_write_then_read_sequences`。`gpt56_cache_write_missing_requests` 只统计新解析器已经看到 usage、但确认没有缓存写入字段的请求；`gpt56_cache_write_unobserved_requests` 统计部署该观测字段前留下的历史记录，不能作为上游漏字段证据。最后一个指标只描述“相同 owner/cache key/model 在 30 分钟内先明确报告写入 0、随后出现缓存读取”的观测序列，不用于反推计费。
...
- Settings/Audit/Auth：`GET/POST/DELETE /admin/ordinary-429-cooldown`（普通 429 冷却，默认 300 秒，可设置 1–86400 秒）、`GET /admin/settings/{key}`、`DELETE /admin/settings/{key}`、`POST /admin/settings/{key}/validate`、`GET /admin/settings/schema`、`GET /admin/audit-logs`、`GET /admin/audit-logs/{id}`、`GET /admin/api-keys`、`POST /admin/api-keys`、`POST /admin/api-keys/{id}/rotate`、`DELETE /admin/api-keys/{id}`。
