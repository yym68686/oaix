...
状态：客户端与应用侧核心观测已实现，发布和操作方式见 [运行说明](egress-observability.md)。调查基线为 `e76bb93`；响应头修复独立提交为 `fff6575`。供应商内部日志和按需节点采集不由应用观测自动提供。
...
本方案先补充观测，保持请求语义、账号选择、计费、冷却、代理配置和在途连接生命周期不变。本次已按用户后续要求进入实现与发布，遵循下述验收门槛。即使补齐客户端观测，也不能保证独立识别第三方代理内部或其远端上游的所有故障；那部分仍需要供应商侧证据。
...
响应头累积问题已在 `fff6575` 单独修复并验收；它不是 EOF 的根因。
...
- [x] 实现客户端阶段、代理配置快照、正文/SSE采集、独立兼容存储和OTLP导出。
- [x] 隔离故障注入、性能、PG14/18迁移及无行为变化验收；详见运行说明中的覆盖边界。
- [ ] 获准后逐步发布并通过自然流量验证。
- [ ] 取得完整因果证据后确定、修复并验证根因。
