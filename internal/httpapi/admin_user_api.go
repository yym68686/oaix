...
func (a *App) platformPoolSummary(w http.ResponseWriter, r *http.Request) {
	...
	sharingCounts, err := a.store.TokenSharingCountsScoped(ctx, store.AllResources())
	if err != nil {
		writeError(w, http.StatusServiceUnavailable, err)
		return
	}
	...
	writeJSON(w, http.StatusOK, map[string]any{"counts": counts, "sharing_counts": sharingCounts, "plan_counts": plans, "error_counts": errorCounts})
}
...
func (a *App) writeTokenList(w http.ResponseWriter, r *http.Request, scope store.ResourceScope) {
	...
	counts, planCounts, adminItems, pendingIDs, err := a.loadTokenListPresentation(r.Context(), scope, opts, asOf, tokens, queryBool(r, "include_quota", false))
	if err != nil {
		writeError(w, http.StatusServiceUnavailable, err)
		return
	}
