...
- `API_KEY_ENCRYPTION_SECRET`: 用户 API Key 可复制明文的服务端加密密钥；未设置时复用 `TOKEN_IMPORT_PAYLOAD_SECRET`，再未设置则从 `DATABASE_URL` 派生。生产环境建议显式配置并保持稳定。
...
- `POST /admin/experiments/responses?token_id=<id>`：管理员专用实验旁路；每次最多一次上游尝试，可用 `fingerprint_enabled=true|false` 或 `fingerprint_mode=off|device|session|full`、`user_agent`、`originator`、`session_id`、`connection_close=true|false`、`disable_compression=true|false`、`http1_only=true|false` 做本次请求覆盖，不改变普通 `/v1/responses` 或账号默认设置
