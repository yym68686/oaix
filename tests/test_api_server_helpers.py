...
def test_request_requires_non_free_codex_token_for_image_tool_and_restricted_models() -> None:
...
def test_execute_proxy_request_with_failover_disables_app_session_terminated_and_retries(monkeypatch) -> None:
    app = SimpleNamespace(
        state=SimpleNamespace(
            response_traffic=ResponseTrafficController(),
            http_client=object(),
    ...
    class DummyOAuthManager:
        ...
        async def get_access_token(
            ...
            *,
            ...
            if token_row.id == 41:
                raise HTTPException(status_code=400, detail=refresh_failure_detail)
            return f"access-token-{token_row.id}", False
    ...
    assert response.status_code == 200
    assert app.state.oauth_manager.invalidated == [41]
    assert claim_calls == [(), (41,)]
    assert mark_error_calls == [
        {
            "token_id": 41,
            "message": refresh_failure_detail,
            "deactivate": True,
            "cooldown_seconds": None,
            "clear_access_token": True,
        }
    ]
    assert proxy_calls == [("access-token-42", "acct_42")]
...
def test_execute_proxy_request_with_failover_disables_access_token_only_auth_failure(monkeypatch) -> None:
    ...
    access_only_token = CodexToken(
        id=51,
        account_id="acct_51",
        refresh_token=f"{ACCESS_TOKEN_ONLY_REFRESH_TOKEN_PREFIX}account:acct_51",
        token_type="codex",
        is_active=True,
    )
    fallback_token = CodexToken(
        id=52,
        account_id="acct_52",
        refresh_token="refresh-52",
        token_type="codex",
        is_active=True,
        last_error="previous error",
    )
    ...
    async def fake_claim_next_active_token(*, selection_strategy: str, exclude_token_ids=None):
        excluded = tuple(sorted(int(token_id) for token_id in (exclude_token_ids or ())))
        claim_calls.append(excluded)
        if access_only_token.id not in excluded:
            return access_only_token
        if fallback_token.id not in excluded:
            return fallback_token
        return None

    async def fake_create_request_log(**kwargs):
        return SimpleNamespace(id=151)
    ...
    assert response.status_code == 200
    assert app.state.oauth_manager.invalidated == [51]
...
def test_execute_proxy_request_with_failover_retries_model_unsupported_codex_token(monkeypatch) -> None:
    ...
    monkeypatch.setenv("MAX_EFFECTIVE_REQUEST_ACCOUNT_RETRIES", "2")
    monkeypatch.delenv("MODEL_COMPATIBILITY_MAX_TOKEN_PROBES", raising=False)
    ...
    bad_tokens = tuple(
        ...
        for token_id in range(151, 160)
    )
    good_token = CodexToken(
        id=160,
        account_id="acct_160",
        refresh_token="refresh-160",
        token_type="codex",
        is_active=True,
        plan_type="plus",
        last_error="previous error",
    ...
    _prompt_cache_bind_lane(app, prompt_cache_context, bad_tokens[0].id)
    ...
    async def fake_claim_next_active_token(
        *,
        ...
        for token in (*bad_tokens, good_token):
            if token.id in excluded:
                continue
    ...
    assert response.status_code == 200
    assert _model_compatibility_max_token_probes() == 10
    assert len(proxy_calls) == 10
    assert proxy_calls[0] == ("access-token-151", "acct_151")
    assert proxy_calls[-1] == ("access-token-160", "acct_160")
    assert len(claim_calls) == 10
    assert claim_calls[0]["scope"] == "gpt-5.3-codex:model-compatibility"
    assert claim_calls[-1]["excluded"] == tuple(range(151, 160))
    assert [call["token_id"] for call in scoped_cooldown_calls] == list(range(151, 160))
    assert {call["scope"] for call in scoped_cooldown_calls} == {"gpt-5.3-codex:model-compatibility"}
    assert mark_success_calls == [160]
    assert finalized[0]["attempt_count"] == 10
    assert finalized[0]["token_id"] == 160
    assert bad_tokens[0].id not in app.state.prompt_cache_affinity_lanes[prompt_cache_context.affinity_key].lanes
...
def test_process_token_import_job_accepts_access_token_only(monkeypatch) -> None:
    completed: dict[str, object] = {}
    access_token = _unsigned_jwt(
        {
            ...
            "https://api.openai.com/auth": {
                "chatgpt_account_id": "acct_access_import",
    ...
    async def fake_refresh_codex_access_token(self, client, refresh_token: str) -> dict[str, object]:
        raise AssertionError("access_token-only imports must not refresh")

    async def fake_publish_token_payload_batch(payloads, *, import_queue_position: str) -> TokenPublishBatchResult:
        assert import_queue_position == "front"
        payload = list(payloads)[0]
        assert payload["access_token"] == access_token
        assert payload["refresh_token"] == f"{ACCESS_TOKEN_ONLY_REFRESH_TOKEN_PREFIX}account:acct_access_import"
        assert payload["token_source"] == "access_token"
        assert payload["expired"].endswith("+00:00")
        token = CodexToken(
            id=42,
            email=None,
            account_id="acct_access_import",
            refresh_token=payload["refresh_token"],
            token_type="codex",
            is_active=True,
        )
    ...
    _install_staged_import_fakes(monkeypatch, 97, [{"access_token": access_token}])
    monkeypatch.setattr(
        "oaix_gateway.token_import_jobs.CodexOAuthManager._refresh_codex_access_token",
        fake_refresh_codex_access_token,
    )
    monkeypatch.setattr("oaix_gateway.token_import_jobs.publish_token_payload_batch", fake_publish_token_payload_batch)
    monkeypatch.setattr("oaix_gateway.token_import_jobs.complete_token_import_job", fake_complete_token_import_job)
    ...
    assert completed["processed_count"] == 1
    assert completed["created"] == [{"id": 42, "email": None, "account_id": "acct_access_import", "index": 0}]
    assert completed["failed"] == []
    assert result is not None
    assert result.created_count == 1
...
def test_probe_token_with_latest_access_token_disables_app_session_terminated(monkeypatch) -> None:
    app = SimpleNamespace(state=SimpleNamespace(http_client=object(), oauth_manager=None))
    token_row = CodexToken(
        id=9,
        account_id="acct_789",
    ...
    async def fake_proxy_request_with_token(*args, **kwargs):
        raise AssertionError("proxy should not run when access-token refresh fails")
    ...
    assert app.state.oauth_manager.invalidated == [9]
    assert calls["error"] == {
        "token_id": 9,
        "message": refresh_failure_detail,
        "deactivate": True,
    ...
    assert "未得出结论" not in result["message"]
